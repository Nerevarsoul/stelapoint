...
def _with_cache(action, cache_key, cache_hours):
    ...
    try:
        ...
        print("Error encountered while loading from cache")
        pass
    ...
    try:
        if sys.version_info.major == 3:
            with open(cache_path, "wb") as cache_file:
                cache_file.write(_bytes(result_str, encoding="utf8"))
        else:
...
def fetch_string(url, num_tries=3, cache_hours=None, use_curl=True, curl_args=None):
    ...
    def download_with_curl():
        with tempfile.TemporaryFile() as temp_file:
            ...
            exit_code = subprocess.call(
                ["curl", "-f", "--retry", str(num_tries), "-A", random.choice(_user_agents), "--silent", "--url", url] + (curl_args or []),
                stdout=temp_file)
            ...
            if contents:
                ...
                raise FetchException("Could not retrieve {0} (return code: {1})".format(url, exit_code))
